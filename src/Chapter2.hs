...
-}
duplicate :: [a] -> [a]
duplicate l = go True l []
  where
    go :: Bool -> [a] -> [a] -> [a]
    go _ [] acc = acc
    go False (x:xs) acc = go True xs (acc ++ [x])
    go True (x:xs) acc = go False (x:xs) (acc ++ [x])
...
takeEven l = go True l []
  where
    go :: Bool -> [a] -> [a] -> [a]
    go _ [] acc = acc
    go False (_:xs) acc = go True xs acc
    go True (x:xs) acc = go False xs (acc ++ [x])
